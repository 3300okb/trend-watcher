---
name: code-review
description: trend-watcher 固有の観点で変更をレビューするチェックリスト。バッチスクリプト（.mjs）・フロントエンド・設定・CI を変更した後の品質確認に使う。
---
...
- ESM のみ（`require()` 不使用、`import.meta.url` でパス解決）。詳細は `scripts/AGENTS.md`。
...
- `public/assets/tailwind.css` は生成物。手編集していないか（ソースは `src/styles/tailwind.css`）。
